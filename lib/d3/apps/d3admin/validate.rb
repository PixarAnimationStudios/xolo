  ...
  module Admin
    ...
    module Validate
      ...
      def validate_existing_package (pkg_to_check)
        ...
        pkgid = D3::Package.ids_to_patchs.invert[pkg_to_check]
        # if not, were we given a title?
        pkgid ||= D3::Package.titles_to_live_ids[pkg_to_check]
      ...
      def validate_title (title)
        raise JSS::NoSuchItemError, "There's no package in d3 with the title '#{title}'" unless D3::Package::Validate.title_exist? title.to_s
        title.to_s
      end
      ...
      def validate_patch (patch)
        D3::Package::Validate.validate_patch patch
      end
      ...
      def validate_expiration_bundle_ids (paths)
        D3::Package::Validate.validate_expiration_bundle_ids paths
      end
