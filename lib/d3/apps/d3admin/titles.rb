...
module D3
  ...
  class AdminApp
    ...
    #
    def add_title
      raise "Title '#{@target} already exists" if D3::Title.exist? @target

      @title = D3::Title.new name: @target, admin: @admin
    ...
    #
    def edit_title
      raise "No such title: '#{@target}" unless D3::Title.exist? @target

      @title = D3::Title.fetch @target
