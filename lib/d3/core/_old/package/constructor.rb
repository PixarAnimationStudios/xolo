  ...
  class Package < JSS::Package
    ...
    def initialize (args={})
      ...
      if args[:id] == :new
        ...
        if D3::Package.all_patchs.include? args[:patch]
      ...
      ########## Looking up an existing package by id, name, title, or patch
        ...
        if args[:id]
          ...
          status =  D3::Package.statuses_by(:patch)[args[:patch]]
          if status
            @status = :missing if status == :missing
            args[:id] = D3::Package.ids_to_patchs.invert[ args[:patch]]
          else
          ...
          args[:id] = D3::Package.titles_to_live_ids[args[:title]]
