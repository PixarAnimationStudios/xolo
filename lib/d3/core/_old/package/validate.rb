...
module D3
  class Package < JSS::Package
    ...
    def check_for_newer_version
      if D3::Client::Receipt.titles.include? @title
          rcpt = D3::Client::Receipt.all[@title]
          raise D3::InstallError, "The installed #{rcpt.patch} (#{rcpt.status}) is the same or newer. Use --force if needed." if rcpt.id >= @id
      end
    ...
    def check_for_deprecated
      raise D3::InstallError, "#{patch} is deprecated. Use --force if needed." if deprecated?
    end
    ...
    def check_for_skipped
      raise D3::InstallError, "#{patch} was skipped. Use --force if needed." if skipped?
    end
    ...
    def check_for_exclusions
      ...
      raise D3::InstallError,  "This machine is excluded for #{patch}. Use --force if needed." unless excl_grps.empty?
      return true
    ...
    def check_oses
       ...
       raise D3::InstallError,  "This machine doesn't have the correct OS to install #{self.patch}." unless JSS.os_ok?  @os_requirements, my_os
       return true
    end
    ...
    def check_cpu
       ...
       raise D3::InstallError,  "This machine doesn't have the correct OS to install #{self.patch}." unless  JSS.processor_ok?  @required_processor, my_cpu
    end
    ...
    module Validate
      ...
      def title_exist? (name)
        D3::Package.all_titles.include? name
      end
      ...
      def patch_exist? (patch)
        D3::Package.all_patchs.include? patch
      end
