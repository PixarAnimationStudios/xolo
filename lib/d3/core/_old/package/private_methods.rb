...
module D3
  class Package < JSS::Package
    ...
    def remove_previous_installs_if_needed (verbose = false)
      if @remove_first && D3::Client::Receipt.titles.include?(@title)
        previous_rcpt = D3::Client::Receipt.all[@title]
        if previous_rcpt.removable?
          D3.log "Uninstalling previously installed version of #{@title}" , :info
          begin
            D3::Client.set_env :uninstalling_before_install, previous_rcpt.patch
      ...
      end # @remove_first && D3::Client::Receipt.titles
    ...
    #
    def write_rcpt
        new_rcpt =  D3::Client::Receipt.new( :title => @title,
            :version => @version,
            :revision => @revision,
            :admin => @admin,
    ...
    def delete_pkg_scripts
      ...
      scripts.each do |script_type|
        ...
        d3_users.map!{|pkgid| D3::Package.ids_to_patchs[pkgid]}
