module D3
  ...
  module Server
    ...
    class App < Sinatra::Base
      ...
      # TODO: move this?
      PACKAGE_UPLOAD_TMP_DIR = Pathname.new "#{D3::Server.config.data_dir}/pkg_uploads"
      ...
      namespace API_V1_ROUTE_BASE do
        namespace PACKAGES_ROUTE_BASE do
          ...
          get '/:package_id' do
            ...
            data = {}
            D3::PACKAGE_ATTRIBUTES.each { |pa| data[pa] = pkg.send(pa) }
            data.to_json
          end
          ...
          post '/?', api_admin_only: true do
            ...
            json_response D3::API_OK_STATUS, 'Created Jamf Package', pkg_name: pkg.name, pkg_id: pkg.id
          end
          ...
          post '/:package_id/upload', api_admin_only: true do
            ...
            json_response(D3::API_OK_STATUS, 'Upload complete')
          end

          # update a pkg in the JSS
          # request body is a JSON hash with D3::PACKAGE_ATTRIBUTES as keys
          #
          put '/:package_id', api_admin_only: true do
            ...
            json_response D3::API_OK_STATUS, 'Updated Jamf Package', pkg_name: pkg.name, pkg_id: pkg.id
          end
          ...
          delete '/:package_id', api_admin_only: true do
            pkg = fetch_package params[:package_id]
            name = pkg.name
            pkg.delete delete_file: true, rw_pw: D3::Server::Helpers::Auth.dist_point_pw, unmount: false

            D3.logger.info "Deleted Jamf Package '#{name}' by #{whodat}"

            json_response D3::API_OK_STATUS, 'Deleted Jamf Package', pkg_name: pkg.name, pkg_id: pkg.id
          end
