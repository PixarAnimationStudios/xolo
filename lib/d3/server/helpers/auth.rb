  ...
  module Server
    ...
    module Helpers
      ...
      module Auth
        ...
        def self.client_pw
          @client_pw ||= pw_from_conf(D3::Server.config.client_pw)
        end

        # load in the pw when first used, and access it here
        def self.jamf_pw
          @jamf_pw ||= pw_from_conf(D3::Server.config.jamf_pw)
        end

        # load in the pw when first used, and access it here
        def self.dist_point_pw
          @dist_point_pw ||= pw_from_conf(D3::Server.config.master_dist_point_rw_pw)
        end
        ...
        def logged_in?
          session[:logged_in] && \
            D3::Server::Helpers::Auth::AUTH_ROLES.include?(session[:role]) && \
        ...
        #   CLIENT_ROLE if user == D3::Server.config.client_acct
        #   ADMIN_ROLE if user != D3::Server.config.client_acct
        #   Halt with error if authentication fails
        #
        def login(user, pw)
          role, failed_msg =
            if user == D3::Server.config.client_acct
        ...
        def authenticate_client(client_pw)
          client_user = D3::Server.config.client_acct
          failed = false
          failed = 'Client authentication failed' unless client_pw == D3::Server::Helpers::Auth.client_pw
        ...
        # D3::Server.config.admin_jamf_group
        #
        ...
        def in_admin_group?(acct_name)
          groupname = D3::Server.config.admin_jamf_group
