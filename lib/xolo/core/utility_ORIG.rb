...
module Xolo
  ...
  # Xolo::CONFIG.client_prohibited_admin_names
  #
  # @return [Array] list of admins not allowed.
  #
  def self.badmins
    return Xolo::DISALLOWED_ADMINS unless Xolo::CONFIG.client_prohibited_admin_names
    Xolo::DISALLOWED_ADMINS + Xolo::CONFIG.client_prohibited_admin_names
  end
  ...
  def self.policy_scripts
    ...
    p_scripts = {}
    res.each { |r| p_scripts[r[0]] = r[1].split(/,\s*/).map(&:to_i) }
    p_scripts
  end
  ...
  # by either the Xolo::Client for lists, or Xolo::Admin for reports,
  ...
  def self.connect_for_reports(api_user, api_pw, db_user, db_pw)
    ...
    if Xolo::CONFIG.report_db_server
      begin
        JSS::DB_CNX.connect(
          server: Xolo::CONFIG.report_db_server,
          user: db_user,
          pw: db_pw,
          timeout: REPORT_CONNECTION_TIMEOUT
        )
        return { db: Xolo::CONFIG.report_db_server, api: JSS::CONFIG.api_server_name }
    ...
    { db: JSS::CONFIG.db_server_name, api: JSS::CONFIG.api_server_name }
  ...
  #
  # See https://infinum.co/the-capsized-eight/multiline-strings-ruby-2-3-0-the-squiggly-heredoc
  #
