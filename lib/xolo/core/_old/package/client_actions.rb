...
module Xolo
  class Package < JSS::Package
    ...
    def run_make_live_script
      ...
      if script = Xolo::CONFIG.admin_make_live_script
    ...
    def install(args = {})
      ...
      raise Xolo::InstallError, "This package is missing from the JSS, cannot install." if @status == :missing
      check_oses
      check_cpu
      ...
      begin  # for the ensure below
        ...
        if args[:puppywalk] and Xolo::PUPPY_Q.q[@title] then
          args[:admin] = Xolo::PUPPY_Q.q[@title].admin ? Xolo::PUPPY_Q.q[@title].admin : "puppy-install"
          args[:force] = Xolo::PUPPY_Q.q[@title].force
          args[:expiration] = Xolo::PUPPY_Q.q[@title].expiration
        else
        ...
        # pilot?
        Xolo::Client.set_env :installing, patch
        Xolo::Client.set_env :pkg_status, @status

        # force?
        @using_force = forced ? " with force" : ""

        # pilot installs need an admin from the args or the queue
        raise JSS::MissingDataError, "Missing :admin for pilot install." if pilot? and @admin == Xolo::AUTO_INSTALL_ADMIN

        # If we aren't actually installing the puppy queue items
        # and there's already a member of this title in the queue
        # raise an exception,
        if Xolo::PUPPY_Q.pups.include? @title && (not args[:puppywalk]) && (not forced)
            raise Xolo::InstallError, "#{@title} (#{Xolo::PUPPY_Q.q[@title].patch}) is already queued for puppies. Use force if needed."
        end
        ...
        #
          ...
          unless forced
            raise Xolo::InstallError, "#{patch} cannot be installed now because one or more of the following processes is running: #{Xolo::Admin::OPTIONS[:prohibiting_processes][:display_conversion].call @prohibiting_processes}."  if install_prohibited_by_process?
          end # unless forced
        ...
        Xolo::Client.unset_env :installing
        Xolo::Client.unset_env :pkg_status
      end # begin...ensure
    end #install
    ...
    #
    def queue_for_puppies (force = Xolo::forced?)

        # create the puppy
        new_pup = Xolo::PuppyTime::PendingPuppy.new( :title => @title,
          :version => @version,
          :revision => @revision,
          :admin => @admin,
          :force => force,
    ...
    def run_pre_install_script(verbose = false)
      ...
      begin
        ...
        Xolo::Client.unset_env :pre_install
      end # begin
      return exit_status
    end # run pre install
    ...
    def run_post_install_script(verbose = false)
      ...
      begin
        ...
        Xolo::Client.unset_env :post_install
      end # begin
      return exit_status
