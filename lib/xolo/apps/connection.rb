...
module Xolo
  ...
  class Connection
    ...
    def connect(user, pw)
      ...
      @user = user
      @cnx_opts[:cookies] = { Xolo::SESSION_KEY => session }
      @restcnx = RestClient::Resource.new @base_url, @cnx_opts
      @connected = true
    end
    alias login connect
    ...
    def initiate_session(user, pw)
      ...
      login_resp.cookies[Xolo::SESSION_KEY]
    end
    ...
    # TODO: needed?
    def session_valid?
      parse_response(@restcnx[SESSION_VALID_RSRC].get)[:status] == Xolo::Server::API_OK_STATUS
    end
    ...
    def validate_connection
      raise Xolo::ConnectionError, 'Not connected' unless connected?
    end
  ...
  def self.cnx
    @connection ||= Xolo::Connection.new
  end
