...
module Xolo
  ...
  class Title < AbstractTitle
    ...
    def self.versions(name)
      validate_title_exists name
      Xolo::Version.all.select { |v| v[:title] == name }
    end
    ...
    # in the Xolo::Criteria object stored in
    # @installed_criteria

    # @param new_ea[Xolo::ExtensionAttribute]
    #
    def add_ext_attr(new_ea)
      raise ArgumentError, 'Parameter must be a Xolo::ExtensionAttribute object' unless new_ea.is_a? Xolo::ExtensionAttribute
...
require 'xolo/apps/title/validation'
