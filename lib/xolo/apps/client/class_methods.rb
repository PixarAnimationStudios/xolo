  ...
  class Client < JSS::Client
    ...
    def self.install(pkgs, options)
      ...
      pkgs.each do |pkg_to_search|
        begin
          # get a Xolo::Package object
          desired_pkg = Xolo::Package.find_package(pkg_to_search)

          raise JSS::NoSuchItemError, "No d3 package matching #{pkg_to_search}" unless desired_pkg
          raise Xolo::InstallError, "The package for #{desired_pkg.patch} is missing from the JSS" if desired_pkg.missing?
          ...
          curr_rcpt = Xolo::Client::Receipt.all[desired_pkg.title]
    ...
    ###
    ### @param pkg[Xolo::Package] the pkg being installed, which might contain an
    ###   admin name
    ###
    ...
    def self.get_admin(pkg_to_install, options)
      ...
      if options.puppies && Xolo::PUPPY_Q.q[pkg_to_install.basename]

        admin = Xolo::PUPPY_Q.q[pkg_to_install.title].admin
        admin ||= Xolo::DFT_PUPPY_ADMIN
    ...
    def self.do_auto_installs(options)
      ...
      begin # for ensure below
        ...
        auto_groups = Xolo::Client.computer_groups.dup
        auto_groups.unshift Xolo::STANDARD_AUTO_GROUP
        auto_groups.each do |group|
          ...
          live_ids_for_group = (Xolo::Package.live_data.keys & Xolo::Package.auto_install_ids_for_group(group))

          live_ids_for_group.each do |live_id|
            # skip those not available
            next unless available_pkg_ids.include? live_id

            auto_install_title = Xolo::Package.live_data[live_id][:title]

            # skip if this title is installed already - it'll be handled with
            # the update_installed_pkgs method during sync.
            next if Xolo::Client::Receipt.all.keys.include? auto_install_title

            new_pkg = Xolo::Package.new id: live_id
        ...
        Xolo::Client.unset_env :auto_install
      end
    end
    ...
    def self.update_installed_pkgs(options)
      ...
      begin # see ensure below
        ...
        live_titles_to_ids = Xolo::Package.titles_to_live_ids
        ...
        Xolo::Client::Receipt.all.values.each do |rcpt|
          ...
          # heres the pkg
          live_pkg = Xolo::Package.new id: live_basenames_to_ids[rcpt.basename]
        ...
        Xolo::Client.unset_env :auto_update
      end # begin..ensure
    ...
    def self.available_pkg_ids(refresh = false)
      ...
      Xolo::Package.package_data.values.each do |pkg|
    ...
    ###   - Xolo::CONFIG.client_try_cloud_distpoint is true
    ...
    def self.cloud_dist_point_to_use(refresh = false, pkg: nil)
      raise 'You must provide a pkg' unless pkg.is_a? Xolo::Package
    ...
    # given a cloud url and a Xolo::Package
    ...
    def self.puppy_in_queue(basename)
      pup = Xolo::PUPPY_Q.queue[basename]
