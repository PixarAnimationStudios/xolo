...
module Xolo
  class Client < JSS::Client
    ...
    #
    # D3 receipts are stored as their native ruby objects in a YAML file located at Xolo::Client::Receipt::DATASTORE
    #
    # When the module loads, the file is read if it exists and all receipts are available in
    #
    # The datastore contains a Hash of Xolo::Client::Receipt objects, keyed by their titles (only one
    ...
    class Receipt
      ...
      # This YAML file stores all Xolo::Client::Receipts on this machine
      DATASTORE = Xolo::SUPPORT_DIR + "receipts.yaml"

      # This locks the loading of receipts when there's a potential to
      # write them back ou.  See Receipt.load_receipts.
      DATASTORE_LOCKFILE = Xolo::SUPPORT_DIR + "receipts.lock"
      ...
      #LAST_APP_USAGE_FILE =  Xolo::SUPPORT_DIR + "last-foreground-times.plist"
      ...
      LAST_APP_USAGE_DIR =  Xolo::SUPPORT_DIR + "Usage"
      ...
      # These args are required when creating a new Xolo::Client::Receipt
      REQUIRED_INIT_ARGS = [
        :title,
        :version,
        :revision,
      ...
      # See Xolo::Client::Receipt.load_receipts and Xolo::Client::Receipt.all
      @@installed_rcpts = nil
      ...
      # Add a Xolo::Client::Receipt to the local rcpt database
      #
      # @param receipt[Xolo::Client::Receipt] the receipt to add
      #
      ...
      # Delete a Xolo::Client::Receipt from the local databse
      #
      # @return [void]
      #
      ...
      # Given a title, patch, or id return the matching Xolo::Receipt
      # or nil if no match.
      ...
      #   the receipt
      #
      # @return [Xolo::Client::Receipt, nil] the matching receipt, if found
      #
      ...
      #
      # @return [Hash{String => Xolo::Client::Receipt}] the receipts for the currently installed pkgs.
      #
      ...
      # Return a hash of Xolo::Client::Receipt
      ...
      # Return a hash of Xolo::Client::Receipt objects for all manually installed
      ...
      def self.rebuild_database
        ...
        Xolo::Package.all.values.each do |d3_pkg|
          ...
          new_rcpts[d3_pkg.title] = Xolo::Client::Receipt.new(:title => d3_pkg.title,
      ...
      def initialize(args = {})
        ...
        unless missing_args.empty?
          raise JSS::MissingDataError, "Xolo::Client::Receipt initialization requires these arguments: :#{REQUIRED_INIT_ARGS.join(', :')}"
        end
        ...
        @manually_installed = (@admin != Xolo::AUTO_INSTALL_ADMIN)
      ...
      # @return [void]
      #
      def uninstall (verbose = false, force = Xolo::forced?)

        raise Xolo::UninstallError,  "#{patch} is not uninstallable" unless self.removable?
        ...
        begin # ...ensure...
          ...
          if uninstall_worked
            ...
            if post_remove_script?
              ...
              if exit_status != 0
                raise Xolo::UninstallError,  "Error running post_remove script (exited #{exit_status}) for #{patch}"
              end
          ...
          Xolo::Client.do_auto_installs(OpenStruct.new) if depiloting

        ensure
          Xolo::Client.unset_env :removing
        end # begin...ensure
      ...
      def uninstall_via_apple_rcpt (verbose = false)
        ...
        begin
          ...
          @apple_pkg_ids.each do |pkgid|
            ...
            raise Xolo::UninstallError, "Error querying pkg file list for '#{pkgid}'" if $CHILD_STATUS.exitstatus > 0
          end # each pkgid
      ...
      # in the receipt based on the matching Xolo::Package data
      #
      # @return [void]
      #
      def repair
      ...
      def update
        Xolo::Client::Receipt.add_receipt(self, :replace)
      end

      # Delete this receipt from the local machine.
      # This removes both the JAMF receipt file, and
      # the Xolo::Client::Receipt from the datastore, and sets @deleted
      # to true.
      #
      # @return [void]
      #
...
Expiration path(s): #{Xolo::Database::ARRAY_OF_PATHNAMES_TO_COMMA_STRING.call @expiration_bundle_ids}
Last brought to foreground: #{last_usage_display}
      ...
      def last_usage
        ...
        fgnd_path = Xolo::Client.foreground_executable_path
        if fgnd_path
      ...
      def status= (new_status)
        raise JSS::InvalidDataError, "status must be one of :#{Xolo::title::STATUSES.join(', :')}" unless Xolo::title::STATUSES.include? new_status
        @status = new_status
        update
      end
