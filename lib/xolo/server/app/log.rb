    ...
    module Log
      ...
      class LogFileWithStream < File
        ...
        def write(str)
          ...
          # Xolo::Server::Log.log_streams.keys.each do |active_stream|
          ...
          #   active_stream << "#{Xolo::Server::Log::LOGSTREAM_DATA_PFX}#{str}\n\n"
          # end
        end # write
      ...
      def self.startup(level = Xolo::Server.config.log_level)
        @logger = make_logger
        ...
        level ||= LOG_LEVELS[Xolo::Server.config.log_level]
        @logger.level = level

        # first startup entry
        @logger.unknown "D3 Server v#{Xolo::VERSION} starting up. PID: #{$PROCESS_ID}, Port: #{Xolo::Server.config.port}"

        # if debug, log our config
        if @logger.level == Logger::DEBUG
          @logger.debug 'Config: '
          Xolo::Server::Config::CONF_KEYS.keys.each do |key|
            @logger.debug "  Xolo::Server.config.#{key} = #{Xolo::Server.config.send key}"
          end
        end
      ...
      def self.make_logger
        # the log file itself
        logfile = LogFileWithStream.new(Xolo::Server.config.log_file, 'a')
        logs_kept = Xolo::Server.config.logs_to_keep

        # return the logger created using a LogFileWithStream instance
        return Logger.new logfile unless logs_kept && logs_kept > 0

        log_size = Xolo::Server.config.log_max_megs * 1024 * 1024
        Logger.new logfile, logs_kept, log_size
      end # make_logger

      # general access to the logger as Xolo::Server::Log.logger.
