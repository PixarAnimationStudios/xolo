 ...
 class App < Sinatra::Base
    ...
    namespace API_V1_ROUTE_BASE do
      ...
      namespace SESSION_ROUTE_BASE do
        ...
        #
        # If the username matches Xolo::Server.config.client_acct then it's a
        ...
        post '/?' do
          ...
          session[:expires] = Time.now + Xolo::Server.config.session_expiration
          json_response(
            Xolo::API_OK_STATUS,
            Xolo::API_LOGGED_IN_MSG,
            role: session[:role],
            user: session[:user]
          )
        end # post
        ...
        delete '/?' do
          ...
          json_response(
            Xolo::API_OK_STATUS,
            Xolo::API_LOGGED_OUT_MSG
          )
        end # delete

        get '/valid' do
          if logged_in?
            json_response Xolo::API_OK_STATUS, Xolo::API_LOGGED_IN_MSG
          else
            json_response Xolo::API_ERROR_STATUS, Xolo::API_NOT_LOGGED_IN_MSG
          end
        end # get valid
