      ...
      module Jamf
        ...
        def self.connect_to_jamf
          @jamf_cnx = JSS.api.connect(
            user: Xolo::Server.config.jamf_acct,
        ...
        def all_pkg_filenames
          rwpw = Xolo::Server::Helpers::Auth.dist_point_pw
        ...
        # Apply a hash of Xolo::PACKAGE_ATTRIBUTES from a
        ...
        def apply_new_pkg_vals(pkg)
          ...
          Xolo::PACKAGE_ATTRIBUTES.each do |attr|
        ...
        def validate_uploaded_pkg(params)
          ...
          halt 400, error_response("Uploaded file must end with #{Xolo::DOT_PKG}") \
            unless name.end_with? Xolo::DOT_PKG
          ...
          Xolo::Server::App::PACKAGE_UPLOAD_TMP_DIR.mkpath
          file_to_store = Xolo::Server::App::PACKAGE_UPLOAD_TMP_DIR + name
          file_to_store.open('wb') { |f| f.write tmpfile.read }

          if Xolo::PKGUTIL.executable?
            path = Shellwords.escape file_to_store.to_s
            halt 400, error_response('Uploaded file is not a valid .pkg') \
              unless system "#{Xolo::PKGUTIL} --payload-files #{path} &>/dev/null"

            if Xolo::Server.config.validate_pkg_signatures
              halt 400, error_response('Uploaded file failed signature validation') \
                unless system "#{Xolo::PKGUTIL} --check-signature #{path} &>/dev/null"
