  ...
  module Admin
    ...
    module Interactive
      ...
      def get_value(option_or_get_method, default = nil, validate_method = nil)
        ...
        if Xolo::Admin::OPTIONS.keys.include?(option_or_get_method)
          option_to_get = option_or_get_method
          get_method = Xolo::Admin::OPTIONS[option_or_get_method][:get]
          # if we weren't giving a validate method, get it from the OPTIONS
          validate_method ||= Xolo::Admin::OPTIONS[option_or_get_method][:validate]
        end
        ...
        until valid === true
          ...
          (valid, validated) = Xolo::Admin::Validate.validate(value_input, validate_method)
      ...
      #
      # If the option is defined in Xolo::Admin::OPTIONS, the data for
      ...
      # Xolo::Admin::OPTIONS
      #
      # If no default value is given in the args, the one from Xolo::Admin::OPTIONS
      ...
      #
      # @param opt[Symbol] The option that is being prompted for, one of the keys of Xolo::Admin::OPTIONS
      #
      ...
      #   the format stored by Xolo::Package objects e.g. an array of groups, a Boolean, nil.
      #   The :display_conversion for that option from Xolo::Admin::OPTIONS will be used to generate the
      ...
      def prompt_for_data (desc: nil, prompt: nil, opt: nil, default: :no_default, required: true )
        ...
        default_display = case default_display
          ...
          when Xolo::Admin::DFT_REQUIRED then '' # the '---Required---' should only be visible in the menu, not the prompt
          when Xolo::Admin::DFT_NONE then UNSET
      ...
      # Ask the user for an patch or title
      # of an existing package.
      #
      # @param default[String, nil] the name to offer as default
      #
      # @return [String,nil] the patch or title entered, or nil
      #
      def get_existing_package(default = nil)
        desc = <<-END_DESC
EXISTING PACKAGE
Enter a package patch or title for an existing d3 package.
If a title, the currently live package for that title will be used.
Enter:
   - 'v' to view a list of all packages with the titles and patchs in d3.
        END_DESC

        input = "v"
        while input == "v" do
          input = prompt_for_data(desc: desc, prompt: "patch or title", default: default, required: true)
          Xolo::Admin::Report.show_all_titles_and_patchs if input == "v"
        end
        return input
...
Enter:
   - 'v' to view a list of all JSS package names not in d3.
        ...
        while input == "v" do
          ...
          Xolo::Admin::Report.show_pkgs_available_for_import if input == "v"
        end
        return input
      end # get existing pkg

      # get a title from the user
      def get_title (default = nil)
        desc = <<-END_DESC
title
Enter a title.
Enter 'v' to view a list of all titles in d3 and
the newest patch for each.
        END_DESC

        input = "v"
        while input == "v" do
          input = prompt_for_data(desc: desc, prompt: "title", required: true)
          Xolo::Admin::Report.show_all_titles_and_patchs if input == "v"
        end
        return input
      end # get title
...
Enter 'v' to view a list of package names currently in d3.
        ...
        while input == "v" do
          ...
          Xolo::Admin::Report.show_existing_package_ids if input == "v"
        end
        return input
      end # get pkg name
...
an existing version of a given title.
        END_DESC
      ...
      def get_description(current_desc = "")
        ...
        if prefd_editor = Xolo::Admin::Prefs.prefs[:editor]
...
END_DESC
      ...
      #
      def get_package_build_type(default = Xolo::Admin::DFT_PKG_TYPE)
        desc = <<-END_DESC
PACKAGE BUILD TYPE
...
E.g. com.mycompany.myapp
      ...
      # title to create the Apple Pkg identifier.
      # For example if the value is com.pixar.d3, then
      # when building a pkg with the title "foo"
      ...
      # @return [String] the prefix to use
      #
      def get_pkg_identifier_prefix (default = Xolo::Admin::DFT_PKG_ID_PREFIX)
              desc = <<-END_DESC
PKG IDENTIFIER PREFIX
Enter the prefix to prepend to a title to create an Apple .pkg indentifier.
E.g. If you enter 'com.mycompany', then when you build a .pkg with title 'foo'
the default .pkg identifier  will be 'com.mycompany.foo'
      ...
      ###
      def get_signing_identity(default = Xolo::Admin::Prefs.prefs[:signing_identity])
        desc = <<-END_DESC
SIGNING IDENTITY
...
as the option for pkgbuild --sign. D3 will not attempt to sign unless this option is set.'
      ...
      ###
      def get_signing_options(default = Xolo::Admin::Prefs.prefs[:signing_options])
        desc = <<-END_DESC
SIGNING OPTIONS
...
Enter '#{Xolo::STANDARD_AUTO_GROUP}' to install on all machines.
        END_DESC
...
Enter 'v' to view list of computer groups.
      ...
      def get_search_target (default = false)
        desc = <<-END_PROMPT
SEARCH TEXT
Enter text to use in matching titles or computer group names.
Matching a title will list all packages with the title.
...
Valid Statuses are: #{Xolo::title::STATUSES_FOR_FILTERS.join(", ")}#{frozen_line}
...
Enter 'y' or 'n'
      ...
      # Ask if we should ininstall older versions of this title
...
Should older versions of this title be uninstalled
...
Enter the number of days, or 0 for no expiration.
      ...
      #
      def get_expiration_bundle_ids (default = 'n')
        desc = <<-END_DESC
EXPIRATION PATH(S)
...
Enter 'y' or 'n'
      ...
      #
      def get_show_type (default = Xolo::Admin::Report::DFT_SHOW_TYPE)
        desc = <<-END_DESC
SERVER PACKAGE LIST
...
END_DESC
        ...
        while input == "v" do
          ...
          Xolo::Admin::Report.show_available_computers_for_reports if input == "v"
        end
        return input
      end # get computer
