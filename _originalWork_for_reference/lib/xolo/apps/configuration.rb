...
module Xolo
  ...
  class Configuration
    ...
    CONF_FILE = Pathname.new '/etc/xolo.conf'
    ...
    #
    def log(msg, level)
      if Xolo.respond_to?(:loaded?) && Xolo.loaded?
        Xolo.log msg, level
      else
        warn "#{level}: #{msg}" if ENV['D3_DEBUG']
      end
    end
    ...
    #
    def read_conf
      read CONF_FILE if CONF_FILE.file? && CONF_FILE.readable?
    end
      ...
      if file.readable?
        ...
        CONF_KEYS.keys.sort.each do |k|
          ...
          # if not, add it to the end unless it's nil
            ...
            data += "\n#{k}: #{savable_value}" unless send(k).nil?
          end # if data =~ /^#{k}:/
        end # each do |k|
    ...
    def to_string(key)
      ...
      str = if self.class.private_method_defined? convert
              send convert
            else
              send(key).to_s
            end
  ...
  CONFIG = Xolo::Configuration.instance
