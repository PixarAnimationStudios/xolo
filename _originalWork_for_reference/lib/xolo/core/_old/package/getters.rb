...
Installation prohibited by process(es): #{Xolo::Admin::OPTIONS[:prohibiting_processes][:display_conversion].call @prohibiting_processes or 'None'}
...
Expiration path(s): #{Xolo::Admin::OPTIONS[:expiration_bundle_ids][:display_conversion].call @expiration_bundle_ids}
